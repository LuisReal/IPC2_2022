...
def Buscar():
    ...
    #print("Se encontre el nombre recibido ",jsonreq)
    nombre_ = request.json['nombre']
    encontrado = VerEmpleadoN(nombre_)
   
    nombre = encontrado[0]
    puesto = encontrado[1]
    salario = encontrado[2]
    depto = encontrado[3]

    #jsonl = {"nombre": nombre, "puesto": puesto, "salario":salario, "departamento": depto}
    jsonl = {"lista": encontrado}
    lista1 = jsonl.get('lista')
    
    print("el nombre es: ", lista1[0])
    return jsonify({"data": jsonl})


def VerEmpleadoN(busca):
    global empleado, nombres, puestos, salarios, departamento
    print(busca)
    encontrado=[]
    cont = 0
    for i in range (len(empleado)):
        for j in range(len(empleado[i])):
            if str(busca) == str(nombres[i][j]):
                encontrado = [nombres[i][j], puestos[i][j], salarios[i][j], departamento[i]]
...
@app.route('/buscarsueldo', methods=["POST"])       
def Buscar3():
    jsonreq = request.get_json()
    print(jsonreq)
    salario_ = jsonreq["salario"]
    VerEmpleadoS(salario_)
    return "listo"


def VerEmpleadoS(busca):
    global empleado, nombres, puestos, salarios
    print(busca)
    cont = 0
    for i in range (len(empleado)):
        for j in range(len(empleado[i])):
            if str(busca) == str(salarios[i][j]):
                print("Nombre: ", nombres[i][j])
                print("Puesto: ", puestos[i][j])
                print("Salario: ", salarios[i][j])
                cont = 1
